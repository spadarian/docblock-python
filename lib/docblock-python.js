...
function getFunctionReturnType(query) {
  ...
  let return_type = match.replace(def_type, '').replace(name, '').trim();
  return return_type;
}
...
function process_def(query, options) {
  ...
  let cython_return_type = this.getFunctionReturnType(query);
  if (query.indexOf('->') > -1 || cython_return_type.length > 0) {
    let return_type;
    if (cython_return_type.length < 1) {
    ...
    if (options.use_types) {
      ...
      if (options.style == 'google') {
        ...
        template.returns = template.returns.filter((x) => {
