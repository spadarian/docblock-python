...
function numpy({name, tabs, use_defaults, default_value, arg_type, type}) {
  return [
    `${tabs}${name} : ${arg_type}\n`,
...
function google({name, tabs, use_defaults, default_value, arg_type, type}) {
  return [
    `${tabs}    ${name} (${arg_type}): Description of parameter \`${name}\``,
    (use_defaults && default_value) ? `. Defaults to ${default_value}` : '',
    '.',
  ].join('');
}

function format_lines(name, tabs, use_defaults, default_value, style, type) {
  let arg_type;
  let lines;
  if (name.indexOf(':') > -1) {
  ...
  if (style == 'numpy') {
    return numpy({name, tabs, use_defaults, default_value, arg_type, type});
  }

  if (style == 'google') {
    return google({name, tabs, use_defaults, default_value, arg_type, type});
  };
