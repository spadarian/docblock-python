...
export default {
  ...
  generate_docblock() {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      if(language === "Python" || language === "MagicPython") {
        var poss = editor.getCursorBufferPositions();
        var poss_ = [];
        var rows_ = [];
        for (p in poss) {
        ...
        poss.map(pos => {
          ...
          var logic = that.accepted_starts.map(function(x) {
            return query.search(x)
          }).some(function(x) {return( x >= 0)});
          if(logic) {
              var header = that.get_header(pos, 0);
              if (!header) {
                atom.notifications.addWarning('Something went wrong. Readeched end of file.')
                return null
              };
              query = header[0];
              ...
              if(query.search('def') == 0) {
                  ...
                  var docblock_and_attrs = that.process_def(query, options);
                  var docblock = docblock_and_attrs[0];
              } else if(query.search('class') == 0) {
                  var docblock = that.process_class(query, options, pos);
              } else {
                  failed += 1;
                  editor.setCursorBufferPosition(pos);
                  if (failed === poss.length) {
                    atom.notifications.addWarning('Nothing to do. Try selecting a line where you define a function or class.')
                  };
              };
  ...
  process_def(query, options) {
    var args = /\(.*\)/.exec(query)[0];
    args = args.slice(1, args.length - 1);
    var args_list = this.scanArgs(args)
                        .filter(function(x) { return x.indexOf('self') < 0 & x.length > 0})
                        .map(function(x) { return x.split('=')[0].trim() });
    var tabs = options.tabs;
    var template = JSON.parse(JSON.stringify(options.template));
    if(args) {
        var that = this;

        args = this.scanArgs(args)
  ...
  get_init(start, lines=1) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      if(last_line.length) {
        ...
        var last_line_n_tabs = 9999; // This is to force the next if to be true
      }
  ...
  get_class_vars(start, init_pos) {
    var new_start = JSON.parse(JSON.stringify(start));;
    var end = JSON.parse(JSON.stringify(init_pos));;
    new_start['row'] += 1;
    end['row'] -= 1;
  ...
  process_class(query, options, pos) {
    ...
    var init_header = this.get_header(init_pos, 0);
    var n_lines = init_header[1];
    ...
    var start_and_args = this.process_def(init_header, class_options);
    var start = start_and_args[0];
    var args_list = start_and_args[1];
    ...
    var class_vars = this.get_class_vars(pos, init_pos);
    ...
    if(attributes.length) {
      ...
      var attrs_text = template.attributes.join(tabs);
