...
function numpy({name, tabs, use_defaults, default_value, use_types, arg_type, type}) {
  return [
    `${tabs}${name} :`,
    (use_types) ? ` ${arg_type}` : '',
    '\n',
...
function google({name, tabs, use_defaults, default_value, use_types, arg_type, type}) {
  return [
    `${tabs}    ${name}`,
    (use_types) ? ` (${arg_type})` : '',
...
function format_lines(name, tabs, use_defaults, default_value, use_types, style, type) {
  let arg_type;
  let lines;
  if (name.indexOf(':') > -1) {
  ...
  if (style == 'numpy') {
    return numpy({name, tabs, use_defaults, default_value, use_types, arg_type, type});
  }

  if (style == 'google') {
    return google({name, tabs, use_defaults, default_value, use_types, arg_type, type});
  };
