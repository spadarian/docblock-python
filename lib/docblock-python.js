...
function format_lines(name, tabs, use_defaults, default_value, use_types, style,
  ...
  } else if (name.indexOf(' ') > -1) { // Cython typing
    let sep_idx = name.indexOf(' ');
    arg_type = name.slice(0, sep_idx).trim();
    name = name.slice(sep_idx + 1).trim();
  } else {
    arg_type = 'type';
  };
  if (style == 'numpy') {
  ...
  if (style == 'sphinx') {
    let separate_types = this.options.separate_types;
    return sphinx({name, tabs, use_defaults, default_value, use_types,
      separate_types, arg_type, type});
  };
...
function format_lines2(params, options, type) {
  let lines = null;
...
function formatLint(msg_pos, par_name, par_type, editorPath) {
  ...
  return {
    ...
    description: `Missing documentation for ${type_text} ${par_name}`,
  };
}
...
function provideLinter() {
  ...
  return {
    ...
    lint(textEditor) {
      ...
      // Do something sync
      // return msg.map((pos) => {
      //   return that.formatLint(pos, editorPath);
      // });
...
function process_def(query, options) {
  ...
  let lines = that.format_lines2(params, options, 'parameter')
    .join('\n') + '\n';
...
function get_all_docblocks() {
  ...
  if (editor = atom.workspace.getActiveTextEditor()) {
    ...
    while (match) {
      ...
      let def = this.get_def({row: start.row, column: 0}, 'up');
      let block = {
        docblock: match[0],
        pos_start_char: match.index,
        pos_start: start,
        pos_end_char: dbRegex.lastIndex,
        pos_end: editor.buffer.positionForCharacterIndex(dbRegex.lastIndex + 1),
        def: def.def[0],
        def_lines: def.def[1],
        def_pos: def.pos,
      };
...
function get_class_init(block) {
  if (editor = atom.workspace.getActiveTextEditor()) {
    ...
    let init_pos = that.find_init(block.pos_end);
